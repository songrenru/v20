...
namespace app\pay\controller\platform;
...
class PayController
{
	public function getPayTypes(){
		$list = (new PayService)->getAllowPaytypes();
	...
	public function getPayTypeInfo(){
		...
		//获取当前支付方式有多少环境
		$group = $pay->getChannelByCode($code);
    	$ret_group = [];
	...
	public function getChannelInfo(){
		...
		if(cfg("open_international_rate") == '1'){
			$output[] = [
				'title' => '费率',
				'name' => 'rate',
				'type' => 'text',
				'value' => $info['rate'],
				'tips' => '费率，设为0则表示不使用该功能'
			];
		}
	...
	public function setChannelParams(){
		...
		if($form_data['open_this']){
			...
			foreach($brothers as $_channel){
				if($_channel && $_channel['channel'] != $_channel['pay_type'] ){
					$brothersIdArr[] = $_channel['id'];
		...
		if($pay->updateChannnelParams($channel_id, $form_data)){
			if($shutdown){
				$pay->shutdownChannels($brothersIdArr);
			}
