...
class HouseVillageCoordinateLog extends Model{
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function getList($where,$field=true,$page=1,$limit=20,$order='l.id DESC')
    {
        $data = $this->alias('l')
            ->leftJoin('house_village_single s','s.area = l.id');
        if($page) {
            $res = $data->where($where)->field($field)->order($order)->page($page, $limit)->select();
            return $res;
        }else{
            $res = $data->where($where)->field($field)->order($order)->select();
