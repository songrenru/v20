...
class HouseVillageLabelCat extends Model{
     ...
     */
    public function getList($where,$field=true,$page=0,$limit=10,$order='cat_id desc')
    {
    ...
    public function getLabelData($where,$field=true,$order='c.cat_id DESC'){
        $data = $this->alias('c')
            ->leftJoin('house_village_label b','c.cat_id = b.cat_id')
            ->where($where)
            ->field($field)
            ->order($order)
            ->select();
