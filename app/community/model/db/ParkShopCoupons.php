...
class ParkShopCoupons extends Model
{
    /**
     * Notes: 获取数量
     * @param $where
    ...
    public function sum($where,$field=true){
        $list = $this->where($where)->sum($field);
        return $list;
    }
    ...
    {
           $sql= $this->alias('mc')
           ->leftJoin('park_coupons c','mc.cid=c.c_id')
           ->leftJoin('park_coupons_shop m','mc.mid=m.m_id')
