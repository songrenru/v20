...
class LifeScenicActivityDetail extends Model
{
    ...
    public function getGoodsInAct($where, $field)
    {
        ...
        $arr = $this->alias('d')
            ->join($prefix . 'life_scenic_activity'.' a' , 'a.id=d.activity_id')
            ->field($field)
            ->where($where)
            ->order('a.sort DESC')
            ->select()->toArray();
    ...
    public function getActDetail($where,$field){
        //找出活动商品列表
        $return = $this->alias('s')
            ->join('life_scenic_activity' . ' la', 'la.id = s.activity_id')
            ->join('life_scenic_limited_act' . ' act', 'act.id = la.act_id')
            ->join('life_scenic_limited_sku' . ' sku', 'sku.ticket_id = s.ticket_id')
        ...
        if(!empty($return)){
            ...
            $ret=$this->getScenicStatus($return);
            ...
            $return['act_stock_num']=(new LifeScenicLimitedSku())->getSum($where_act,'act_stock_num');
     ...
     * 活动列表
     */
    public function getScenicStatus($arr)
    {
        $left = array();
...
//                        $left['limited_status'] = 0;
//                        $left['left_time']=86400-$arr['cycle_start_time'];
//                    }
//                }
