...
class FloorImport extends Command
{
    ...
    {
        // 指令配置
        $this->setName('floorImport')
            ->addArgument('json',Argument::IS_ARRAY)
            ->setDescription('楼层命令行导入指令');
    }

    //临时打印日志
    private function floorImportFdump($data, $filename='test', $append=false){
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        $this->floorImportFdump(['接收参数==' . __LINE__, $json, $params], 'import_excel/floorImportExecute', 1);
        ...
        try {
            ...
            $this->floorImportFdump(['$processCacheKey设置失败==' . __LINE__, $processCacheKey, $processInfo, $params, $e->getMessage()], 'import_excel/floorImportExecuteError', 1);
        }
        ...
        //↓↓↓表格读取数据开始↓↓↓
        $this->floorImportFdump(['colAndFileds'=>$colAndFileds], 'import_excel/000unitImport', 1);
        for ($startRow = 2; $startRow <= $highestRow; $startRow += $chunkSize) {
            $chunkFilter->setRows($startRow, $chunkSize);
            $spreadsheet = $reader->load($inputFileName);
            $sheetData = $spreadsheet->getSheetByName($worksheetName)->toArray(NULL, true, true, true);
            $this->floorImportFdump(['sheetData'=>$sheetData], 'import_excel/000unitImport', 1);
            $excelTitle = $sheetData[1];
            unset($sheetData[1]); //去掉表头
            //开始业务逻辑
            foreach ($sheetData as $dk => $data) {
                if ((isset($data[$colAndFileds['layer_name']]) && empty($data[$colAndFileds['layer_name']]))
                    && (isset($data[$colAndFileds['layer_number']]) && empty($data[$colAndFileds['layer_number']]))
                ...
                //楼层编号
                $layer_number  = isset($data[$colAndFileds['layer_number']]) ? $data[$colAndFileds['layer_number']] : null;
                //楼栋名称
                ...
                if (empty($layer_name) && empty($layer_number) && empty($single_name) && empty($floor_name) && empty($sort)){
                    $this->floorImportFdump(['数据为空=='.__LINE__,$data],'import_excel/unitImportExecuteError',1);
                    continue;
                }
                //楼栋名称
                ...
                if (!$floor){
                    $data[$nextIndex]= "该单元名称【{$floor_name}】不存在！";
                    $allErrorData[] =  $data;
                    $errorTotal += 1;
                    continue;
                }
                // 查询是否有同一楼层名称
                $check_layer=$NewBuildingService->getHouseVillageLayerFindMethod([
                    ['village_id','=',$village_id],
                    ['single_id','=',$single['id']],
                ...
                if($check_layer && $isRepeated){
                    $data[$nextIndex]= "该楼层名称【{$layer_name}】已存在！";
                    $allErrorData[] =  $data;
                    $errorTotal += 1;
                    continue;
                }
                // 查询是否有同一楼层编号
                if(!$layer_number){
                    $layer_number=$NewBuildingService->checkNumberMethod($layer_name);
                    $check_layer=$NewBuildingService->getHouseVillageLayerFindMethod([
                        ['village_id','=',$village_id],
                        ['single_id','=',$single['id']],
                        ['floor_id','=',$floor['floor_id']],
                    ...
                    if($check_layer && $isRepeated){
                        $data[$nextIndex]= "该楼层编号【取楼层名称后2位】已存在，请填写其他 1 - {$max_num} 号的楼层编号！";
                        $allErrorData[] =  $data;
                        $errorTotal += 1;
                        continue;
                    }
                    ...
                    if($check_layer && $isRepeated){
                        $data[$nextIndex]= "该楼层编号【{$layer_number}】已存在，请填写其他 1 - {$max_num} 号的楼层编号！";
                        $allErrorData[] =  $data;
                        $errorTotal += 1;
                        continue;
                    }
                ...
                if($check_layer  && !$isRepeated){  //更新
                    $update_id=$check_layer['id'];
                    ...
                    $update_status=$NewBuildingService->saveAllHouseVillageLayerMethod($update_id,$updateData);
                    $this->floorImportFdump(['覆盖数据=='.__LINE__,$replaceField,$update_id,$updateData,$update_status],'import_excel/floorImportExecuteUpdate',1);
                }
                else{ //新增
                    $allInsertData[]  = [
                        'layer_name'    => $layer_name,
...
//            sleep(1);
        ...
        if($allInsertData){ //写入楼层数据
            $NewBuildingService->addAllHouseVillageLayerMethod($allInsertData);
        }
        $this->floorImportFdump(['处理完成=='.__LINE__,'总花费时间:'.$second.'s',$record,$res,$allErrorData,$allInsertData],'import_excel/floorImportExecute',1);
        return  true;
