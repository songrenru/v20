<?php
/**
 * 计划任务 待支付预订单 15分钟 超时取消
 * Author: hengtingmei
 * Date Time: 2020/07/01 17:48
 */

namespace app\common\model\service\plan\file;
use app\foodshop\model\service\order\DiningOrderRefundService;
use app\foodshop\model\service\order\DiningOrderService;
class DiningOrderCancleService {
    /**
	 * 计划任务执行
	 * @param  string  $param 参数
	 */
 
	public function runTask(){
        fdump('OrderCancleService','v20_plan',1);
        $time = 900;
        // v20 餐饮
        $where = [
            ['is_book_pay', '<>', 1],
            ['book_pay_time', '=', 0],
            ['status', '=', 2],
            ['order_from', '=', 0],
            ['create_time', '<', time()-$time],
        ];
        fdump($where,'v20_plan',1);
        $diningOrderService = new DiningOrderService();
        $diningOrderRefundService = new DiningOrderRefundService();
        $diningList = $diningOrderService->getOrderListByCondition($where, [], 0);
        fdump('diningList','v20_plan',1);
        fdump($diningList,'v20_plan',1);
        foreach ($diningList as $key => $value) {
            $value['cancel_reason'] = L_('超时自动取消');
            $diningOrderRefundService->cancelOrderAndGoods($value);
        }

        // 简易模式 自取 超时取消
        $where = [
            ['status', '=', 3],
            ['order_from', '=', 4],
            ['create_time', '<', time()-$time],
        ];
        $diningOrderService = new DiningOrderService();
        $diningOrderRefundService = new DiningOrderRefundService();
        $diningList = $diningOrderService->getOrderListByCondition($where, '', 0);
        fdump('简易模式','v20_plan',1);
        fdump($diningList,'v20_plan',1);
        foreach ($diningList as $key => $value) {
            $value['cancel_reason'] = L_('超时自动取消');
            $diningOrderRefundService->cancelOrderAndGoods($value);
        }
        
		return true;
	}
}