...
use file_handle\FileHandle; //自动判断是否要上传到OSS
...
abstract class Driver
{
     ...
     * @return bool|string
     */
    public function putFile(string $path, File $file, $rule = null, array $options = [], $type=false)
    {
        return $this->putFileAs($path, $file, $file->hashName($rule), $options,$type);
    }
     ...
     * @param array  $options 参数
     * @param string  $type 参数 企业微信加
     * @return bool|string
     */
    public function putFileAs(string $path, File $file, string $name, array $options = [], $type=false)
    {
		...
		if(!$type) {
            //自动判断上传OSS
            if($result){
                $object = app()->getRootPath() . '../upload/' . $path;
                $file_handle = new FileHandle();
                $upload_result = $file_handle->upload($object);
            }
        }
