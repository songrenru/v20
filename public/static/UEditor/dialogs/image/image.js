    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            function addFile(file) {
                var $li = $('<li class="item" id="' + file.id + '">' +
            ...
            uploader.on('uploadSuccess', function (file, ret) {
                ...
                try {
                    ...
                    if (json.state == 'SUCCESS') {
                        //放入一个图片id作为标识 每当上传完成一张图片时，就将拖拽数据设置为空数组
                        sortableFileList = [];
                        let jsons = {
                            ...json,
                            id: file.id
                        }
                        _this.imageList.push(jsons);
        ...
        getInsertList: function () {
            var i, data, list = [],
                ...
                //按顺序拖动图片
                //sortableFileList存在就说明拖动过
                console.log(sortableFileList,'sortableFileList');
                console.log(this.imageList,'this.imageList');
                if(sortableFileList.length>0){
                    sortableFileList.forEach(v=>{
                        data = this.imageList.find(s=>s.id == v)
                        list.push({
                            src: prefix + data.url,
