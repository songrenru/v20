...
class OssClient
{
    ...
    public function __construct($accessKeyId, $accessKeySecret, $endpoint, $isCName = false, $securityToken = NULL, $requestProxy = NULL)
    {
		...
		if (empty($accessKeyId)) {
            throw new OssException("开了阿里云OSS，需要在管理后台填写“阿里云AccessKeyId”参数");
        }
        if (empty($accessKeySecret)) {
            throw new OssException("开了阿里云OSS，需要在管理后台填写“阿里云AccessKeySecret”参数");
        }
        if (empty($endpoint)) {
            throw new OssException("开了阿里云OSS，需要在管理后台填写“OSS EndPoint”参数");
        }
     ...
     * @throws OssException
     */
    private function precheckBucket($bucket, $errMsg = '开了阿里云OSS，需要在管理后台填写“Bucket 名称”参数')
    {
