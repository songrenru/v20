...
class WxPayApi
{
	...
	/**
	 * 普通商户文档地址：https://pay.weixin.qq.com/wiki/doc/api/jsapi.php?chapter=9_1
	 * 服务商文档地址：https://pay.weixin.qq.com/wiki/doc/api/jsapi_sl.php?chapter=7_3
	 ...
	 * 
	 * 请求单次分账
	 * out_refund_no、transaction_id、receiver为必填参数
	 * appid、mchid、spbill_create_ip、nonce_str不需要填入
	 * @param WxPayConfigInterface $config  配置对象
	...
	public static function ProfitSharing($config, $inputObj, $timeOut = 6)
	{
		...
		//检测必填参数
		if(!$inputObj->GetOut_order_no()) {
			throw new WxPayException("分账接口中，缺少必填参数out_order_no！");
		}else if(!$inputObj->IsTransaction_idSet()){
			throw new WxPayException("分账接口中，缺少必填参数transaction_id！");
		}else if(!$inputObj->GetReceivers()){
			throw new WxPayException("分账接口中，缺少必填参数receiver！");
		}
		...
		$inputObj->SetSignType('HMAC-SHA256');//签名类型
		$inputObj->SetSign($config);//签名
		$xml = $inputObj->ToXml();
		$startTimeStamp = self::getMillisecond();//请求开始时间
		$response = self::postXmlCurl($config, $xml, $url, true, $timeOut);
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function Profitsharingaddreceiver($config, $inputObj, $timeOut = 30)
	{
		$url = "https://api.mch.weixin.qq.com/secapi/pay/profitsharingaddreceiver";
		//检测必填参数
		if(!$inputObj->GetReceiver()){
			throw new WxPayException("分账接口中，缺少必填参数receiver！");
		}
	 ...
	 * 
	 * 完成分账
	 * out_refund_no、transaction_id为必填参数
	 * appid、mchid、spbill_create_ip、nonce_str不需要填入
	 * @param WxPayConfigInterface $config  配置对象
	 * @param WxPayProfitSharing $inputObj
	 * @param int $timeOut
	...
	{		
		...
		curl_setopt($ch,CURLOPT_SSL_VERIFYHOST,FALSE);//关闭严格校验
		...
		if($useCert == true){
			...
			fdump($sslCertPath, "lumin_refu", 1);
			fdump($sslKeyPath, "lumin_refu", 1);
			fdump($url, "lumin_refu", 1);
		...
		if($data){
			...
			curl_close($ch);
			if($error=='58'){
				throw new WxPayException("微信支付证书异常，请检查证书配置！");
			}else{
				throw new WxPayException("curl出错，错误码:【".$error."】。查看错误码对应地址：http://curl.haxx.se/libcurl/c/libcurl-errors.html");
			}
